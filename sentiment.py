...
def train_model(df, tokenizer, model, device):
    train_texts, val_texts, train_labels, val_labels = train_test_split(
        df['review_text'].tolist(), df['label'].tolist(), test_size=0.2, random_state=42, stratify=df['label'])
...
def load_saved_model():
    ...
    model = BertForSequenceClassification.from_pretrained('./sentiment_model', num_labels=3)
    return tokenizer, model


def save_model(model, tokenizer, model_path="sentiment_model", tokenizer_path="sentiment_tokenizer"):
